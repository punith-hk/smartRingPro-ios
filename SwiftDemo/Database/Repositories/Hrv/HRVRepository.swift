...
/// HeartRate Repository (equivalent to Android's HrvRepository)
/// Handles all database operations for heart rate data
class HrvRepository {
    
    private let context: NSManagedObjectContext
    private let TAG = "HrvRepository"
    ...
    /// Save new batch of HRV readings from BLE device
    /// Equivalent to Android's saveNewBatch(readings: List<Pair<Long, String>>)
    func saveNewBatch(readings: [(timestamp: Int64, hrvValue: Int)], completion: @escaping (Bool, Int) -> Void) {
        ...
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            guard let self = self else {
                print("[HrvRepository] ⚠️ Self is nil in background task")
                return
            }
            ...
            do {
                ...
                print("[\(self.TAG)] ✅ Saved \(newReadings.count) new HRV entries")
                DispatchQueue.main.async {
    ...
    func getAll() -> [HRVEntity] {
        ...
        do {
            ...
            print("[\(TAG)] 📊 Fetched \(results.count) HRV entries")
            return results
        } catch {
    ...
    /// Delete all HRV data
    func deleteAll() {
        CoreDataManager.shared.deleteAllData(for: "HrvEntity")
    }
    ...
    /// Print all saved HRV data (for debugging)
    func printAllData() {
        let all = getAll()
        ...
        for (index, entry) in all.enumerated() {
            ...
            print("  \(index + 1). HRV: \(entry.hrvValue)ms | Time: \(dateStr) | Timestamp: \(entry.timestamp)")
        }
        print("----------------------------------------")
    }
    ...
    func printSummary() {
        ...
        let hrvValues = all.map { Int($0.hrvValue) }
        let minHrv = hrvValues.min() ?? 0
        let maxHrv = hrvValues.max() ?? 0
        let avgHrv = hrvValues.reduce(0, +) / hrvValues.count
        ...
        print("  Min HRV: \(minHrv)ms")
        print("  Max HRV: \(maxHrv)ms")
        print("  Avg HRV: \(avgHrv)ms")
