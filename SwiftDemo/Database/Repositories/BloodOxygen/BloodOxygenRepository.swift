...
/// BloodOxygen Repository (equivalent to Android's BloodOxygenRepository)
/// Handles all database operations for blood oxygen data
class BloodOxygenRepository {
    
    private let context: NSManagedObjectContext
    private let TAG = "BloodOxygenRepository"
    ...
    /// Save new batch of blood oxygen readings from BLE device
    /// Equivalent to Android's saveNewBatch(readings: List<Pair<Long, String>>)
    func saveNewBatch(readings: [(timestamp: Int64, oxygenValue: Int)], completion: @escaping (Bool, Int) -> Void) {
        ...
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            guard let self = self else {
                print("[BloodOxygenRepository] ⚠️ Self is nil in background task")
                return
            }
            ...
            for reading in newReadings {
                BloodOxygenEntity.create(
                    timestamp: reading.timestamp,
                    oxygenValue: Int16(reading.oxygenValue),
                    batchTime: batchTime,
            ...
            do {
                ...
                print("[\(self.TAG)] ✅ Saved \(newReadings.count) new blood oxygen entries")
                DispatchQueue.main.async {
    ...
    /// Get all blood oxygen entries ordered by timestamp descending
    func getAll() -> [BloodOxygenEntity] {
        let request = BloodOxygenEntity.fetchAll()
        
        do {
            let results = try context.fetch(request)
            print("[\(TAG)] 📊 Fetched \(results.count) blood oxygen entries")
            return results
        } catch {
    ...
    /// Get latest blood oxygen entry
    func getLatestEntry() -> BloodOxygenEntity? {
        let request = BloodOxygenEntity.fetchLatest()
    ...
    func getLatestBatch() -> [BloodOxygenEntity] {
        let request = BloodOxygenEntity.fetchAll()
    ...
    /// Get blood oxygen entries for today
    func getTodayLatestEntry() -> BloodOxygenEntity? {
        ...
        let request = BloodOxygenEntity.fetchByDateRange(start: startTimestamp, end: endTimestamp)
    ...
    /// Get blood oxygen entries for specific date range
    func getByDateRange(start: Date, end: Date) -> [BloodOxygenEntity] {
    ...
    /// Delete all blood oxygen data
    func deleteAll() {
        CoreDataManager.shared.deleteAllData(for: "BloodOxygenEntity")
    }
    ...
    /// Print all saved blood oxygen data (for debugging)
    func printAllData() {
        let all = getAll()
