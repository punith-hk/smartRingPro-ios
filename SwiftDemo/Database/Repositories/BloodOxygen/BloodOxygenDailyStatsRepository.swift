...
/// Repository for daily aggregated heart rate statistics
/// Stores data from API endpoint: getRingDataByDay
class BloodOxygenDailyStatsRepository {
    
    private let context: NSManagedObjectContext
    private let TAG = "BloodOxygenDailyStatsRepository"
    ...
    func saveDailyStats(userId: Int, date: String, value: String, completion: @escaping (Bool) -> Void) {
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            ...
            let fetchRequest = BloodOxygenDailyStatsEntity.fetchByUserAndDate(userId: userId, date: date, in: backgroundContext)
            
            do {
                let results = try backgroundContext.fetch(fetchRequest)
                
                let entity: BloodOxygenDailyStatsEntity
    ...
    func saveBatch(userId: Int, stats: [(date: String, value: String)], completion: @escaping (Bool, Int) -> Void) {
        ...
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            ...
            for stat in stats {
                let fetchRequest = BloodOxygenDailyStatsEntity.fetchByUserAndDate(userId: userId, date: stat.date, in: backgroundContext)
    ...
    func getAllForUser(userId: Int) -> [BloodOxygenDailyStatsEntity] {
        let request = BloodOxygenDailyStatsEntity.fetchAllForUser(userId: userId)
    ...
    func getRecentDays(userId: Int, days: Int) -> [BloodOxygenDailyStatsEntity] {
        let request = BloodOxygenDailyStatsEntity.fetchAllForUser(userId: userId)
    ...
    func getByDate(userId: Int, date: String) -> BloodOxygenDailyStatsEntity? {
        let request = BloodOxygenDailyStatsEntity.fetchByUserAndDate(userId: userId, date: date, in: context)
    ...
    func deleteAllForUser(userId: Int) {
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: "BloodOxygenDailyStatsEntity")
    ...
    func deleteAll() {
        CoreDataManager.shared.deleteAllData(for: "BloodOxygenDailyStatsEntity")
    }
}
