...
/// Repository for daily aggregated temperature statistics
/// Stores data from API endpoint: getRingDataByDay
class TemperatureDailyStatsRepository {
    
    private let context: NSManagedObjectContext
    private let TAG = "TemperatureDailyStatsRepository"
    ...
    func saveDailyStats(userId: Int, date: String, value: String, diastolicValue: String, completion: @escaping (Bool) -> Void) {
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            ...
            let fetchRequest = TemperatureDailyStatsEntity.fetchByUserAndDate(userId: userId, date: date, in: backgroundContext)
            
            do {
                let results = try backgroundContext.fetch(fetchRequest)
                
                let entity: TemperatureDailyStatsEntity
                if let existing = results.first {
                    ...
                    // Create new
                    entity = TemperatureDailyStatsEntity(context: backgroundContext)
    ...
    func saveBatch(userId: Int, stats: [(date: String, value: String, diastolicValue: String)], completion: @escaping (Bool, Int) -> Void) {
        ...
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            ...
            for stat in stats {
                let fetchRequest = TemperatureDailyStatsEntity.fetchByUserAndDate(userId: userId, date: stat.date, in: backgroundContext)
    ...
    func getAllForUser(userId: Int) -> [TemperatureDailyStatsEntity] {
        let request = TemperatureDailyStatsEntity.fetchAllForUser(userId: userId)
    ...
    func getRecentDays(userId: Int, days: Int) -> [TemperatureDailyStatsEntity] {
        let request = TemperatureDailyStatsEntity.fetchAllForUser(userId: userId)
    ...
    func getByDate(userId: Int, date: String) -> TemperatureDailyStatsEntity? {
        let request = TemperatureDailyStatsEntity.fetchByUserAndDate(userId: userId, date: date, in: context)
    ...
    func deleteAllForUser(userId: Int) {
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: "TemperatureDailyStatsEntity")
    ...
    func deleteAll() {
        CoreDataManager.shared.deleteAllData(for: "TemperatureDailyStatsEntity")
    }
}
