...
/// Repository for daily aggregated blood pressure statistics
/// Stores data from API endpoint: getRingDataByDay
class BloodPressureDailyStatsRepository {
    
    private let context: NSManagedObjectContext
    private let TAG = "BloodPressureDailyStatsRepository"
    ...
    func saveDailyStats(userId: Int, date: String, value: String, diastolicValue: String, completion: @escaping (Bool) -> Void) {
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            ...
            let fetchRequest = BloodPressureDailyStatsEntity.fetchByUserAndDate(userId: userId, date: date, in: backgroundContext)
            
            do {
                let results = try backgroundContext.fetch(fetchRequest)
                
                let entity: BloodPressureDailyStatsEntity
                if let existing = results.first {
                    ...
                    // Create new
                    entity = BloodPressureDailyStatsEntity(context: backgroundContext)
    ...
    func saveBatch(userId: Int, stats: [(date: String, value: String, diastolicValue: String)], completion: @escaping (Bool, Int) -> Void) {
        ...
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            ...
            for stat in stats {
                let fetchRequest = BloodPressureDailyStatsEntity.fetchByUserAndDate(userId: userId, date: stat.date, in: backgroundContext)
    ...
    func getAllForUser(userId: Int) -> [BloodPressureDailyStatsEntity] {
        let request = BloodPressureDailyStatsEntity.fetchAllForUser(userId: userId)
    ...
    func getRecentDays(userId: Int, days: Int) -> [BloodPressureDailyStatsEntity] {
        let request = BloodPressureDailyStatsEntity.fetchAllForUser(userId: userId)
    ...
    func getByDate(userId: Int, date: String) -> BloodPressureDailyStatsEntity? {
        let request = BloodPressureDailyStatsEntity.fetchByUserAndDate(userId: userId, date: date, in: context)
    ...
    func deleteAllForUser(userId: Int) {
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: "BloodPressureDailyStatsEntity")
    ...
    func deleteAll() {
        CoreDataManager.shared.deleteAllData(for: "BloodPressureDailyStatsEntity")
    }
}
