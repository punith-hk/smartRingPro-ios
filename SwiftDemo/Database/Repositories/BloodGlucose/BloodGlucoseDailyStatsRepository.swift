...
class BloodGlucoseDailyStatsRepository {
    
    private let context: NSManagedObjectContext
    private let TAG = "BloodGlucoseDailyStatsRepository"
        ...
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            ...
            do {
                ...
                if let existing = results.first {
                    ...
                    // Create new
                    entity = BloodGlucoseDailyStatsEntity(context: backgroundContext)
    ...
    func saveBatch(userId: Int, stats: [(date: String, value: String, diastolicValue: String)], completion: @escaping (Bool, Int) -> Void) {
        ...
        CoreDataManager.shared.performBackgroundTask { [weak self] backgroundContext in
            ...
            for stat in stats {
                let fetchRequest = BloodGlucoseDailyStatsEntity.fetchByUserAndDate(userId: userId, date: stat.date, in: backgroundContext)
    ...
    func getAllForUser(userId: Int) -> [BloodGlucoseDailyStatsEntity] {
        let request = BloodGlucoseDailyStatsEntity.fetchAllForUser(userId: userId)
    ...
    func getRecentDays(userId: Int, days: Int) -> [BloodGlucoseDailyStatsEntity] {
        let request = BloodGlucoseDailyStatsEntity.fetchAllForUser(userId: userId)
    ...
    func getByDate(userId: Int, date: String) -> BloodGlucoseDailyStatsEntity? {
        let request = BloodGlucoseDailyStatsEntity.fetchByUserAndDate(userId: userId, date: date, in: context)
    ...
    func deleteAllForUser(userId: Int) {
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: "BloodGlucoseDailyStatsEntity")
    ...
    func deleteAll() {
        CoreDataManager.shared.deleteAllData(for: "BloodGlucoseDailyStatsEntity")
    }
}
