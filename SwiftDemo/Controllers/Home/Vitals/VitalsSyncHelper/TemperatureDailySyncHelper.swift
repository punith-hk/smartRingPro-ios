...
/// Helper to sync daily aggregated Temperature data
/// Flow: Load from local DB → Show UI → Fetch from API → Compare → Update DB if changed → Reload UI
class TemperatureDailySyncHelper {
    
    protocol TemperatureDailySyncListener: AnyObject {
    ...
    private weak var listener: TemperatureDailySyncListener?
    private let TAG = "TemperatureDailySyncHelper"
    private let repository: TemperatureDailyStatsRepository
    
    private var isSyncing = false
    
    init(listener: TemperatureDailySyncListener) {
        self.listener = listener
        self.repository = TemperatureDailyStatsRepository()
    }
    ...
    private func loadFromLocalDB(userId: Int) -> [TemperatureDailyStatsEntity] {
    ...
    private func processAPIData(userId: Int, apiData: [GetRingDataByDayResponse.DayData]) {
        ...
        let localDict = Dictionary(uniqueKeysWithValues: localStats.compactMap { stat -> (String, TemperatureDailyStatsEntity)? in
    ...
    private func convertToDataPoints(_ stats: [TemperatureDailyStatsEntity]) -> [VitalDataPoint] {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd"
