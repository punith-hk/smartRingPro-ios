...
/// Helper to sync daily aggregated Blood Glucose data
/// Flow: Load from local DB → Show UI → Fetch from API → Compare → Update DB if changed → Reload UI
class BloodGlucoseDailySyncHelper {
    
    protocol BloodGlucoseDailySyncListener: AnyObject {
    ...
    private weak var listener: BloodGlucoseDailySyncListener?
    private let TAG = "BloodGlucoseDailySyncHelper"
    private let repository: BloodGlucoseDailyStatsRepository
    
    private var isSyncing = false
    
    init(listener: BloodGlucoseDailySyncListener) {
        self.listener = listener
        self.repository = BloodGlucoseDailyStatsRepository()
    }
    ...
    private func loadFromLocalDB(userId: Int) -> [BloodGlucoseDailyStatsEntity] {
    ...
    private func processAPIData(userId: Int, apiData: [GetRingDataByDayResponse.DayData]) {
        ...
        let localDict = Dictionary(uniqueKeysWithValues: localStats.compactMap { stat -> (String, BloodGlucoseDailyStatsEntity)? in
    ...
    private func convertToDataPoints(_ stats: [BloodGlucoseDailyStatsEntity]) -> [VitalDataPoint] {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd"
