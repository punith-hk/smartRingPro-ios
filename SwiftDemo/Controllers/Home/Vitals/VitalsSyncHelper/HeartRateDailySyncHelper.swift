...
/// Helper to sync daily aggregated heart rate data
/// Flow: Load from local DB → Show UI → Fetch from API → Compare → Update DB if changed → Reload UI
class HeartRateDailySyncHelper {
    
    protocol HeartRateDailySyncListener: AnyObject {
    ...
    private weak var listener: HeartRateDailySyncListener?
    private let TAG = "HeartRateDailySyncHelper"
    private let repository: HeartRateDailyStatsRepository
    
    // Track if API sync is in progress
    private var isSyncing = false
    
    init(listener: HeartRateDailySyncListener) {
        self.listener = listener
        self.repository = HeartRateDailyStatsRepository()
    }
    ...
    private func loadFromLocalDB(userId: Int) -> [HeartRateDailyStatsEntity] {
    ...
    private func processAPIData(userId: Int, apiData: [GetRingDataByDayResponse.DayData]) {
        ...
        let localDict = Dictionary(uniqueKeysWithValues: localStats.compactMap { stat -> (String, HeartRateDailyStatsEntity)? in
        ...
        if statsToSave.isEmpty {
            print("[\(TAG)] ✅ Local DB is up to date, no changes needed")
            return
        }
        ...
        repository.saveBatch(userId: userId, stats: statsToSave) { [weak self] (success: Bool, savedCount: Int) in
            ...
            if success && savedCount > 0 {
                print("[\(self.TAG)] 💾 Saved \(savedCount) new/updated entries to local DB")
                
                // Reload data from DB and notify UI
                ...
                print("[\(self.TAG)] ⚠️ No changes saved to DB")
            }
        }
    }
    ...
    /// Convert database entities to VitalDataPoint for chart
    private func convertToDataPoints(_ stats: [HeartRateDailyStatsEntity]) -> [VitalDataPoint] {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd"
