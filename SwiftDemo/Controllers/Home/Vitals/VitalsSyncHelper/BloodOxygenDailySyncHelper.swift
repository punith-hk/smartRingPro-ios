...
/// Helper to sync daily aggregated Blood Oxygen data
/// Flow: Load from local DB → Show UI → Fetch from API → Compare → Update DB if changed → Reload UI
class BloodOxygenDailySyncHelper {
    
    protocol BloodOxygenDailySyncListener: AnyObject {
    ...
    private weak var listener: BloodOxygenDailySyncListener?
    private let TAG = "BloodOxygenDailySyncHelper"
    private let repository: BloodOxygenDailyStatsRepository
    
    private var isSyncing = false
    
    init(listener: BloodOxygenDailySyncListener) {
        self.listener = listener
        self.repository = BloodOxygenDailyStatsRepository()
    }
    ...
    private func loadFromLocalDB(userId: Int) -> [BloodOxygenDailyStatsEntity] {
    ...
    private func processAPIData(userId: Int, apiData: [GetRingDataByDayResponse.DayData]) {
        ...
        let localDict = Dictionary(uniqueKeysWithValues: localStats.compactMap { stat -> (String, BloodOxygenDailyStatsEntity)? in
    ...
    private func convertToDataPoints(_ stats: [BloodOxygenDailyStatsEntity]) -> [VitalDataPoint] {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd"
